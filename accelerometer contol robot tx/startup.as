	...
	; Compiler options:
	;
	; -oCEP0896.cof -mCEP0896.map --summary=default --output=default \
	; CEP0896.p1 --chip=16F877A -P --runtime=default --opt=default \
...
;	20 bytes of RAM objects in bank 0 to zero

	psect	init
	global	__Lrbss_0
	movlw	low(__Lrbss_0)		;load the low address into FSR
	movwf	FSR
	movlw	low(__Lrbss_0+20)		;load the high address+1 into W
...
;	6 bytes of RAM objects in bank 1 to zero

	psect	init
	global	__Lrbss_1
	movlw	low(__Lrbss_1)		;load the low address into FSR
	movwf	FSR
	movlw	low(__Lrbss_1+6)		;load the high address+1 into W
	fcall	clear_ram		;go and zero that range of addresses
